...
// DEPRECATED — replaced by the internal voting system in Phase 3 of the
// voting-system rewrite. Removed entirely once all auto-creation flows land.
//
...
export const POST: RequestHandler = async ({ request, locals, params }) => {
	...
	try {
		const { snapshotProposalId, snapshotProposalLink } = await request.json();
